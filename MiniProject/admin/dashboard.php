...
?>
    ...
    <?php
        $count_student = mysqli_query($link,"SELECT * FROM `student_information`");
        $total_student = mysqli_num_rows($count_student);   //Student table er koyta row asey seta rakhbe

        $count_user = mysqli_query($link,"SELECT * FROM `users`");
        $total_user = mysqli_num_rows($count_user); //User table a koyta user row asey seta rakhbe
    ?>
